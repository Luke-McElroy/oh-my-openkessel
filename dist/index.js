...
export const RalphPlugin = async (ctx) => {
        ...
        tool: {
            ralph_init: tool({
                ...
                async execute(args, toolCtx) {
                    ...
                    const result = await planAgent.initialize(args.projectGoal, args.techStack, args.constraints);
            ...
            ralph_memory: tool({
                ...
                async execute(args, toolCtx) {
                    ...
                    if (args.keyword) {
                        ...
                        results = results.filter(e => e.keywords.some(k => k.toLowerCase().includes(keyword)));
                    }
                    ...
                    const output = results.map(e => `Memory ${e.id}: Phase ${e.phase}, Iter ${e.iteration} - ${e.status} - ${e.keywords.join(', ')}`).join('\n');
