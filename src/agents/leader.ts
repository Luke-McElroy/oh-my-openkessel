...
import type { Phase, HandoffMessage, AgentResult, Memory, RalphState, SessionState, OutOfScopeFlag } from '../types/index.js';
...
export class LeaderAgent {
  ...
  async initialize(assumeMode: boolean = false): Promise<void> {
  ...
  private async runPhase(phase: Phase): Promise<'continue' | 'halt'> {
  ...
  private async attemptRescope(phase: Phase): Promise<void> {
  ...
  private async createHandoffMessage(phase: Phase): Promise<HandoffMessage> {
    return {
      phaseId: phase.id,
  ...
  private async insertPhase(newPhase: Phase): Promise<void> {
    const phases = await fileUtils.loadPhases(this.ctx.$, this.projectPath);
  ...
  private async writeMemory(phase: Phase, status: 'pass' | 'fail', notes: string, flags: OutOfScopeFlag[]): Promise<void> {
  ...
  private extractKeywords(phase: Phase, notes: string, flags: OutOfScopeFlag[]): string[] {
  ...
  private async getPhaseMemories(phaseId: number): Promise<Memory[]> {
    const index = await fileUtils.loadMemoriesIndex(this.ctx.$, this.projectPath);
  ...
  private async getPriorMemories(phaseId: number): Promise<string[]> {
    const index = await fileUtils.loadMemoriesIndex(this.ctx.$, this.projectPath);
    return index
