...
import type { Phase, Memory, RalphState } from '../types/index.js';
...
export class PlanAgent {
  ...
  async initialize(projectGoal: string, techStack?: string, constraints?: string): Promise<string> {
...
|----|-------|-----------|----------|--------|------|`;
  ...
  async rescope(phaseId: number, failureContext: string, memories: Memory[]): Promise<string> {
  ...
  async insertPhase(afterPhaseId: number, phaseSuggestion: Partial<Phase>): Promise<string> {
    const phases = await fileUtils.loadPhases(this.ctx.$, this.projectPath);
  ...
  private async generatePRD(projectGoal: string, techStack?: string, constraints?: string): Promise<string> {
...
Generate the full PRD content now.`;
  ...
  private async generatePhases(projectGoal: string, techStack?: string, constraints?: string): Promise<Phase[]> {
...
[
  {
    ...
    if (jsonMatch) {
      const phaseData = JSON.parse(jsonMatch[0]) as Array<{
        name: string;
        goal: string;
        tasks: string[];
  ...
  private async regeneratePhase(phase: Phase, failureContext: string, memories: Memory[]): Promise<Phase> {
    const memoriesText = memories.map(m => 
      `Memory ${m.id}: ${m.context}\nAttempted: ${m.attempted}\nFailed: ${m.failed}`
    ).join('\n\n');
...
Return ONLY a JSON object in this format:
{
  ...
  private async getOrCreateSession(): Promise<string> {
    ...
    const existing = sessions.find((s: { title?: string }) => s.title?.includes('ralph-plan'));
