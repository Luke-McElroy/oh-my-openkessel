...
import type { Phase, HandoffMessage, AgentResult, AgentResult as ReviewResult, OutOfScopeFlag } from '../types/index.js';
...
export class ReviewerAgent {
  ...
  async execute(handoff: HandoffMessage, buildResult: AgentResult, testResult: AgentResult): Promise<AgentResult> {
  ...
  async evaluateOutOfScopeFlags(handoff: HandoffMessage, flags: OutOfScopeFlag[]): Promise<AgentResult> {
...
Return format:
{
  ...
  private formatPrompt(handoff: HandoffMessage, buildResult: AgentResult, testResult: AgentResult, phase?: Phase): string {
...
Return format:
{
  ...
  private async getOrCreateSession(): Promise<string> {
    ...
    const existing = sessions.find((s: { title?: string }) => s.title?.includes('ralph-reviewer'));
